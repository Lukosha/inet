...
long parseBucketSize(const char *attrValue, const char *attrName, cSimpleModule &owner, int defaultValue)
{
    ...
    char *unit;
    double size = strtod(attrValue, &unit);
    return 8 * (long)cNEDValue::convertUnit(size, unit, "B");
}
...
{
    if (!strcasecmp(attrValue, "green"))
        return IMeter::GREEN;
    else if (!strcasecmp(attrValue, "yellow"))
        return IMeter::YELLOW;
    else if (!strcasecmp(attrValue, "red"))
        return IMeter::RED;
    else
        return parseIntAttribute(attrValue, attrName, false);
}
...
void parseColors(const char *attrValue, const char *attrName, int numColors, std::vector<int> &result)
{
    ...
    else
    {
        cStringTokenizer tokens(attrValue);
        while(tokens.hasMoreTokens())
            result.push_back(parseColor(tokens.nextToken(), attrName));
    }
}
...
std::string colorToString(int color)
{
    switch(color)
    {
        case IMeter::GREEN: return "green";
        case IMeter::YELLOW: return "yellow";
        case IMeter::RED: return "red";
        default: return intToString(color);
    }
}
